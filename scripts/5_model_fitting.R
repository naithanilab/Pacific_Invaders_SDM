...
foreach(spec = specs, .packages = c("tidyverse", "wrswoR", "raster", "sf", "mecofun", "dismo", "mgcv", "gbm", "randomForest", "blockCV", "sperrorest")) %dopar% {
  ...
  try({
    ...
    if(nrow(occ_thinned) < 30){          # Skip species with too few occurrences
      return()
    }
    ...
    abs_thinned = thin_coordinates_sample(abs_coords) # 5 km minimum threshold
