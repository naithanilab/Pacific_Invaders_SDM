...
- step 1 - I intersected the cleaned occurrences with GIFT shapefile and got the "geoentity" ID-s of the smallest intersected polygons for each occurrence (some fall offshore);
...
$\Box$ Need to check why many GIFT ID-s are not shared. Especially those that are in Pacific Invaders cleaned file but not in the occurrence dataset.
...
$\boxed{\checkmark}$ The alien status info needs to be prepared. For that reason, I created the script `data_prep_clean_alien_status_data.R` This produces a clean alien status that should be used next in merging with the dt_oc that was intersected with the GIFT shapefile. Also the script `data_prep_merge_alien_status_to_occ.R` should be renamed to `data_prep_intersect_gift.R` and keep there only the intersection. The merging should be done then in a 3rd script maybe called exactly `data_prep_merge_alien_status_to_occ.R` (this script will be short actually).
...
According to Mike, merge this file using column `island` with the file `googlenames_gift.csv`. And then use the column `ID_1` for matching with Patrick's shapefile `geoentities_2018-05-09` (GIFT).
...
Also, even if it sounds counterintuitive, do not use the column `invasive` from `Pacific_Invaders_0707.csv`. Actually ALL records in this file are "exotic". So, any raw from the occurence data that cannot be match with this file, gets an `NA`. Anything that can be matched by species names and GIFT ID, is "alien/exotic/invasive".
...
**Malakal** has no entry in `geoentities_2018-05-09.shp`, but could be assigned to `ge_ntty` *Koror*, `entt_ID` 1664 (note that there is another *Koror*, *11682*, but a bit bigger). Malakal Island is also "Ngemelachel" in Palauan, but didn't find any "Ngemelachel" entry in GIFT.
